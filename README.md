...
Some RRBS protocols include methylated and unmethylated spike-in sequences to be used as conversion controls. They can be analyzed using this bundle of scripts [conversionCrt.tar.gz](http://www.biomedical-sequencing.at/bocklab//papers/klughammer2015/resources/conversionCtr.tar.gz). These scripts expect the environment modules `samtools/1.3`, `bismark/0.12.2`, and `bowtie/1.1.1`. These might need to be adapted to the respective environment. Other versions might not work.
